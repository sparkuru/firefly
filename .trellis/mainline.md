...
| 10 | `.trellis/tasks/08-27-repository-audit-remediation/` — P1 repository audit remediation | active | The Unicode compatibility prerequisite, documentation-convergence, deterministic-validation, and comments-contract children are archived. X Core/route cleanup is now the current planning child; adapter cleanup and release/observability hardening remain separately verifiable follow-up children with the dependencies recorded in the parent PRD. |
