...
- port 80 `staging.SOURCE-DOMAIN` redirecting to its HTTPS URL;
- port 443 TLS using the existing `SOURCE-DOMAIN` wildcard certificate;
...
already listening and that no current configuration owns `staging.SOURCE-DOMAIN`.
