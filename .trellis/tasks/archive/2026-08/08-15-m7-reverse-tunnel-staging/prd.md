...
staging TLS/Nginx edge while the static runtime remains on the owner
...
- The owner-supplied SSH alias accepts strict-host-key, key-auth login. The
  remote account has non-interactive `sudo`.
...
- The staging name has wildcard-DNS/CDN coverage but no dedicated remote Nginx
  server block. At the origin, its HTTP request redirects to HTTPS and its
  HTTPS request returns 404.
- The origin's active wildcard certificate can serve the staging name without
  issuing or storing a new certificate.
...
- R3: Add an isolated staging Nginx server block that preserves HTTP
...
- R6: Prove rollback by removing the dedicated Nginx configuration, stopping
  ...
  closed and the staging name no longer serves the rehearsed release.
- R7: Store no credentials, private keys, CDN/edge material, host IPs, or
...
- [x] The staging HTTPS route reaches the exact assembled release through
