...
`staging.SOURCE-DOMAIN` TLS/Nginx edge while the static runtime remains on the owner
...
- `ssh wkyuu@ssh.SOURCE-DOMAIN` accepts strict-host-key, key-auth login. The remote
...
- `staging.SOURCE-DOMAIN` has wildcard-DNS/CDN coverage but no dedicated remote Nginx
  server block. At the origin, its HTTP request redirects to HTTPS and its
  HTTPS request returns 404.
- The origin's active certificate is valid for `SOURCE-DOMAIN` and `*.SOURCE-DOMAIN`; it
  can serve `staging.SOURCE-DOMAIN` without issuing or storing a new certificate.
...
- R3: Add an isolated `staging.SOURCE-DOMAIN` Nginx server block that preserves HTTP
...
- R6: Prove rollback by removing the dedicated Nginx configuration, stopping
  ...
  closed and `staging.SOURCE-DOMAIN` no longer serves the rehearsed release.
...
- [x] `https://staging.SOURCE-DOMAIN/` reaches the exact assembled release through
