...
1. Confirm a clean local worktree, strict SSH host-key verification, active
   remote Nginx, unused `staging.SOURCE-DOMAIN` server name, and unused remote
   loopback port 9450.
...
4. Upload/install the one isolated `staging.SOURCE-DOMAIN` Nginx file. Validate with
...
- TLS validates for `staging.SOURCE-DOMAIN`; HTML retains the runtime security/cache
