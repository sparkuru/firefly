...
- The repository contains no staging hostname, remote access details, TLS
  ...
  specific remote host or `SOURCE-DOMAIN` domain.
...
- Local Docker preflight on 2026-08-14 confirms the existing runtime model:
  ...
  staging target.
- Owner supplied `ssh wkyuu@ssh.SOURCE-DOMAIN` as the staging-host control path and
  confirmed `*.SOURCE-DOMAIN` wildcard DNS behavior. A strict-host-key, key-auth SSH
  ...
  was found, and `staging.SOURCE-DOMAIN` resolves from the host. A later read-only
...
- Superseded decision: a remote `staging.SOURCE-DOMAIN` origin and its associated
...
- Remote `SOURCE-DOMAIN` binding, a staging VM, SSH deployment, public DNS, and public
