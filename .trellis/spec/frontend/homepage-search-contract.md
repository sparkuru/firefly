...
- The labeled mobile form and inline results section are siblings of Terminal
  startup, recovery, and session sections. Placing search only inside recovery
  would hide it on successful Terminal startup.
...
- Reveal the search section only after its own initialization succeeds. No
  ...
  failure path. The search-failed recovery CSS must outrank connecting-state and
  hidden recovery rules so browse links remain visible both before and after
  successful Terminal startup; setting `fallback.hidden = false` alone is not
  sufficient against author CSS.
...
- Native search controls and result links remain protected from Terminal typing
  capture/shortcuts. Searching does not replace the transcript, replay startup,
  or reconstruct browse/session visibility. The inline connecting-state Escape
  guard also leaves events targeted within the search section native; protecting
  only the ready controller would miss this early input path.
- On eligible clients with visible search, the homepage is a column with a
  viewport minimum height; an empty Terminal session fills the remaining area
  beneath the search form/results rather than reserving another full viewport.
  Clearing Terminal centers its prompt in that remaining area without extra
  page overflow. Desktop keeps its original viewport-center behavior. If a
  delayed controller reveals search before transferring the boot log, the whole
  log's origin may move by the measured search height/margins; internal line,
  typography, width, and prompt geometry still remain unchanged. On tall touch
  viewports that independent header also translates startup/short-output reading
  band placement; regression measurements account for exactly its rendered
  height/margins. Desktop's header contribution remains zero.
...
| Terminal startup succeeds/fails | Search remains independently usable on eligible clients |
...
- Base: a title substring narrows public posts/pages, and Clear removes results
  without changing the Terminal transcript.
...
- Test built artifacts for touch form discovery before/after boot, submission,
  clear, IME, no-results, native result navigation, protected Terminal input,
  startup/search failure, media changes, public-only data, body extraction
  boundaries, overflow/target/focus behavior, and no-JavaScript recovery.
...
```astro
<!-- Wrong: successful Terminal startup hides this surface. -->
<section data-terminal-fallback>
  <form role="search">...</form>
</section>

<!-- Correct: search visibility belongs to its independent controller. -->
